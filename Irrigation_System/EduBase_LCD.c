...
void EduBase_LCD_Ports_Init(void)
{
	...
	//Configure the PC6 pins as output by setting Bit 6 in the DIR register
	GPIOC-> DIR |= 0x40;
	...
	//Initialize the output of te PC6 pin to zero
	...
	//R4 bit (Bit 4) in the RCGCPIO register
...
void EduBase_LCD_Pulse_Enable(void)
{
	...
	//during a read / write operation (page 49 of the HD44780 datasheet)
	GPIOC->DATA |= 0x40;
	SysTick_Delay1us(1);
	GPIOC->DATA &= ~0x40;
...
void EduBase_LCD_Write_4_Bits(uint8_t data, uint8_t control_flag)
{
	...
	//Set of clear the register select (RS) pin based on the control flag
	...
	//Clear the LCD data lines (PA2 - PA5) and provide a 1ms delay
	GPIOA->DATA &= ~0x3C;
...
void EduBase_LCD_Init(void)
{
	...
	//Transmit a Return Home command to set the DDRAM address to 0 and to retun
	//the display from being shifter to the original position
	...
	//Turn off the cursor and the blinking of the cursor position character
